...
JOIN LATERAL (
   ...
   WHERE  pi.payload_list_id = pl.id
     AND (pi.major_iov < %(my_major_iov)s OR (pi.major_iov = %(my_major_iov)s AND pi.minor_iov <= %(my_minor_iov)s))
   ORDER BY pi.major_iov DESC NULLS LAST, pi.minor_iov DESC NULLS LAST
   LIMIT 1
) pi ON true
JOIN "PayloadType" pt ON pl.payload_type_id = pt.id;
'''
get_payload_iovs_lateral_major = '''
SELECT pt.name AS payload_type_name, pi.payload_url, pi.major_iov, pi.minor_iov
FROM "PayloadList" pl
JOIN "GlobalTag" gt ON pl.global_tag_id = gt.id AND gt.name = %(my_gt)s
JOIN LATERAL (
   ...
   FROM   "PayloadIOV" pi
   WHERE  pi.payload_list_id = pl.id
     AND pi.major_iov <= %(my_major_iov)s
   ORDER BY pi.major_iov DESC NULLS LAST, pi.minor_iov DESC NULLS LAST
   LIMIT 1
) pi ON true
JOIN "PayloadType" pt ON pl.payload_type_id = pt.id;
'''
get_payload_iovs_lateral_minor = '''
SELECT pt.name AS payload_type_name, pi.payload_url, pi.major_iov, pi.minor_iov
FROM "PayloadList" pl
JOIN "GlobalTag" gt ON pl.global_tag_id = gt.id AND gt.name = %(my_gt)s
JOIN LATERAL (
   ...
   FROM   "PayloadIOV" pi
   WHERE  pi.payload_list_id = pl.id
     AND pi.minor_iov <= %(my_minor_iov)s
   ORDER BY pi.major_iov DESC NULLS LAST, pi.minor_iov DESC NULLS LAST
   LIMIT 1
) pi ON true
JOIN "PayloadType" pt ON pl.payload_type_id = pt.id;
'''
get_payload_iovs_lateral_comb = '''
SELECT pt.name AS payload_type_name, pi.payload_url, pi.major_iov, pi.minor_iov
FROM "PayloadList" pl
JOIN "GlobalTag" gt ON pl.global_tag_id = gt.id AND gt.name = %(my_gt)s
JOIN LATERAL (
