...
class RuleEngine:
    ...
    def _rule_r9_locked_gt_high_read_volume(self, context: RuleContext) -> list[Suggestion]:
        if (
            context.global_tag_name
            and context.has_locked_gt
            and context.calls > 1000
            ...
            return [
                Suggestion(
                    ...
                    priority="MEDIUM",
                    message=(
                        f"High read volume for locked GlobalTag '{context.global_tag_name}' detected. "
                        "Consider precomputing latest valid IOV per PayloadType via materialized view."
                    ),
                    safe_sql=None,
                    confidence=0.8,
                )
